...
\begin{frame}
    ...
    \texttt{L4\_ThreadId\_t New(in String path, in String args);}
...
\begin{frame}
    ...
    \begin{itemize}
        \item \emph{Task server} asks \emph{memory server} to create a new \emph{address space} 
        \item \emph{Task server} creates a new \emph{region mapper} inside new \emph{address space}
        \item \emph{Task server} asks \emph{memory server} to map old \emph{User space} and \emph{Stack} into new \emph{address space}
...
\begin{frame}
    ...
    \texttt{L4\_ThreadId\_t StartThread(in L4\_Pointer\_t ip, in L4\_Pointer\_t sp, in L4\_Word\_t stackSize);}
...
\begin{frame}
    ...
    \texttt{void SetStatisticInterval(in unsigned int interval);}
...
\begin{frame}
    ...
    \texttt{void SetTimeslice(in unsigned int timeslice);}
...
\begin{frame}
    ...
    \texttt{unsigned int SetPriority(in unsigned int priority);}
...
\begin{frame}
    ...
    \texttt{unsigned int Ping(in L4\_ThreadId\_t tid);}

    \begin{itemize}
        \item Sends \emph{ping message} to \emph{region mapper} of \emph{address space} to check if it is still responsive
    \end{itemize}
\end{frame}
